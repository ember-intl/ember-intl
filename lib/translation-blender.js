...
TranslationBlender.prototype.report = function (target, defaultLocaleProps, locale) {
    var targetProps = propKeys(target);

    defaultLocaleProps.forEach(function (property) {
...
TranslationBlender.prototype.updateCache = function (includePaths, destDir) {
    ...
    var defaultLocale      = this.readAsObject(path.join(includePath, this.options.defaultLocale + '.json'));
    var defaultLocaleProps = propKeys(defaultLocale);
    ...
    walkSync(includePath).forEach(function (file) {
        ...
        this.report(translation, defaultLocaleProps, locale);

        translation = extend(true, {}, defaultLocale, translation);
