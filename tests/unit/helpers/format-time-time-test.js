...
moduleForComponent('format-datetime-time', {
  integration: true,
  beforeEach() {
...
test('invoke formatDateTime directly', function(assert) {
  assert.expect(1);

  const output = service.formatDateTime(date, { timeZone: 'UTC', locale: 'fr-fr' });

  // Try both for browser Intl data inconsistencies
  assert.ok(output === '23/1/2014' || output === '23/01/2014');
});

test('invoke formatDateTime directly with format', function(assert) {
  assert.expect(1);

  registry.register(
    'formats:main',
    { datetime: { test: { timeZone: 'UTC', locale: 'fr-fr' } } },
    { instantiate: false }
  );

  const output = service.formatDateTime(date, { format: 'test' });
...
test('it should return a formatted string from a date string', function(assert) {
  ...
  this.render(hbs`{{format-datetime dateString timeZone='UTC'}}`);
