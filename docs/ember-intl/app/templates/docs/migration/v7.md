...
- The `t()` method no longer uses `assert()` to check that `key` (the 1st positional argument) is `string`. `options.default` has no effect and should be replaced with an `if` or `if-else` statement.
