...
### Removed `app/formats.js` support {#breaking-changes-removed-app-formats-js-support}
...
### Renamed `{{format-relative}}` {#breaking-changes-renamed-format-relative}
...
When a translation is missing, the `intl` service uses [`setOnMissingTranslation()`](../services/intl-part-2#methods-set-on-missing-translation) to display some string. The default value for development and production has been `Missing translation "${key}" for locale "${locale}"`. The value is different for testing.
...
### `{{t-key}}` helper {#features-t-key-helper}
