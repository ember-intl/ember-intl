...
> [!IMPORTANT]
>
> After installing `@ember-intl/v1-compat`, you don't need to reconfigure your app to load translations.
>
...
> [!IMPORTANT]
>
> After installing `@ember-intl/vite`, you need to reconfigure your app to load translations. One, replace `config/ember-intl.js` with `ember-intl.config.{js,mjs}`. Two, update `vite.config.{mjs,mts}` and `tsconfig.json` so that you can import a "virtual" file.
>
