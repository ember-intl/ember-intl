...
describe('traverse', function() {
  ...
  it('{name} took {numPhotos, plural, =0 {no photos} =1 {one photo} other {# photos}} on {takenDate, date, long}.', function() {
    let ast = messageParser.parse(`{name} took {numPhotos, plural, =0 {no photos} =1 {one photo} other {# photos}} on {takenDate, date, long}.`);
  ...
  it('{ gender, select, male {He avoids bugs} female {She avoids bugs} other {They avoid bugs} }', function() {
    let ast = messageParser.parse(`{ gender, select, male {He avoids bugs} female {She avoids bugs} other {They avoid bugs} }`);
  ...
  it('{ trainers, plural, offset:1 ... }', function() {
    let ast = messageParser.parse(`{ trainers, plural, offset:1
         =0 {The gym is empty}
  ...
  it(`It's my cat's {year, selectordinal, ... }`, function() {
    let ast = messageParser.parse(`It's my cat's {year, selectordinal,
        one {#st}
        two {#nd}
        few {#rd}
        other {#th}
...
function count(ast) {
  ...
  traverse(ast, {
    messageFormatPattern() { result.messageFormatPattern++; },
    messageTextElement() { result.messageTextElement++; },
    argumentElement() { result.argumentElement++; },
    numberFormat() { result.numberFormat++; },
    dateFormat() { result.dateFormat++; },
    timeFormat() { result.timeFormat++; },
    pluralFormat() { result.pluralFormat++; },
    selectFormat() { result.selectFormat++; },
    optionalFormatPattern() { result.optionalFormatPattern++; },
  });
