# @ember-intl/lint

## 2.1.0

### Minor Changes

- [#2212](https://github.com/ember-intl/ember-intl/pull/2212) Allowed ignores using regex ([@ijlee2](https://github.com/ijlee2))

### Patch Changes

- [#2211](https://github.com/ember-intl/ember-intl/pull/2211) Refactored LintRunWithIgnores ([@ijlee2](https://github.com/ijlee2))
- [#2211](https://github.com/ember-intl/ember-intl/pull/2211) Added timestamp to discourage caching the user config across tests ([@ijlee2](https://github.com/ijlee2))

## 2.0.3

### Patch Changes

- [#2208](https://github.com/ember-intl/ember-intl/pull/2208) Updated dependencies ([@ijlee2](https://github.com/ijlee2))

## 2.0.2

### Patch Changes

- [#2205](https://github.com/ember-intl/ember-intl/pull/2205) Updated compatibility in README ([@ijlee2](https://github.com/ijlee2))

## 2.0.1

### Patch Changes

- [#2201](https://github.com/ember-intl/ember-intl/pull/2201) Updated dependencies ([@ijlee2](https://github.com/ijlee2))

## 2.0.0

### Major Changes

- [#2185](https://github.com/ember-intl/ember-intl/pull/2185) Removed handling nested translation JSON ([@ijlee2](https://github.com/ijlee2))
- [#2184](https://github.com/ember-intl/ember-intl/pull/2184) Removed handling spaces in translation folder names ([@ijlee2](https://github.com/ijlee2))
- [#2179](https://github.com/ember-intl/ember-intl/pull/2179) Removed inputPath, publicOnly, and wrapTranslationsWithNamespace ([@ijlee2](https://github.com/ijlee2))
- [#2175](https://github.com/ember-intl/ember-intl/pull/2175) Dropped support for Node 20 ([@ijlee2](https://github.com/ijlee2))

### Patch Changes

- [#2195](https://github.com/ember-intl/ember-intl/pull/2195) Updated dependencies ([@ijlee2](https://github.com/ijlee2))
- [#2193](https://github.com/ember-intl/ember-intl/pull/2193) Updated dependencies ([@ijlee2](https://github.com/ijlee2))
- [#2187](https://github.com/ember-intl/ember-intl/pull/2187) Updated dependencies ([@ijlee2](https://github.com/ijlee2))
- [#2183](https://github.com/ember-intl/ember-intl/pull/2183) Updated dependencies ([@ijlee2](https://github.com/ijlee2))
- [#2182](https://github.com/ember-intl/ember-intl/pull/2182) Supported the syntax `@service()` ([@ijlee2](https://github.com/ijlee2))
- [#2180](https://github.com/ember-intl/ember-intl/pull/2180) Updated dependencies ([@ijlee2](https://github.com/ijlee2))

## 1.5.1

### Patch Changes

- [#2182](https://github.com/ember-intl/ember-intl/pull/2182) Supported the syntax `@service()` ([@ijlee2](https://github.com/ijlee2))

## 1.5.0

### Minor Changes

- [#2171](https://github.com/ember-intl/ember-intl/pull/2171) Supported buildOptions keys namespaceKeysByDir and translationsDir (@ember-intl/lint) ([@ijlee2](https://github.com/ijlee2))

### Patch Changes

- [#2173](https://github.com/ember-intl/ember-intl/pull/2173) Updated dependencies ([@ijlee2](https://github.com/ijlee2))

## 1.4.0

### Minor Changes

- [#2168](https://github.com/ember-intl/ember-intl/pull/2168) Updated dependencies (eslint@v10, typescript@v6) ([@ijlee2](https://github.com/ijlee2))

## 1.3.1

### Patch Changes

- [#2166](https://github.com/ember-intl/ember-intl/pull/2166) Updated dependencies ([@ijlee2](https://github.com/ijlee2))

## 1.3.0

### Minor Changes

- [#2164](https://github.com/ember-intl/ember-intl/pull/2164) Updated dependencies ([@ijlee2](https://github.com/ijlee2))

## 1.2.1

### Patch Changes

- [#2162](https://github.com/ember-intl/ember-intl/pull/2162) Updated dependencies ([@ijlee2](https://github.com/ijlee2))

## 1.2.0

### Minor Changes

- [#2161](https://github.com/ember-intl/ember-intl/pull/2161) Updated dependencies ([@ijlee2](https://github.com/ijlee2))

## 1.1.3

### Patch Changes

- [#2149](https://github.com/ember-intl/ember-intl/pull/2149) Updated dependencies ([@ijlee2](https://github.com/ijlee2))

## 1.1.2

### Patch Changes

- [#2143](https://github.com/ember-intl/ember-intl/pull/2143) Updated dependencies ([@ijlee2](https://github.com/ijlee2))

## 1.1.1

### Patch Changes

- [#2140](https://github.com/ember-intl/ember-intl/pull/2140) Updated dependencies ([@ijlee2](https://github.com/ijlee2))

## 1.1.0

### Minor Changes

- [#2138](https://github.com/ember-intl/ember-intl/pull/2138) Fixed lint errors ([@ijlee2](https://github.com/ijlee2))

## 1.0.1

### Patch Changes

- [#2137](https://github.com/ember-intl/ember-intl/pull/2137) Updated README ([@ijlee2](https://github.com/ijlee2))
- [#2136](https://github.com/ember-intl/ember-intl/pull/2136) Updated dependencies ([@ijlee2](https://github.com/ijlee2))

## 1.0.0

### Major Changes

- [#2134](https://github.com/ember-intl/ember-intl/pull/2134) Stable release ([@ijlee2](https://github.com/ijlee2))

### Minor Changes

- [#2132](https://github.com/ember-intl/ember-intl/pull/2132) Allowed --fix to update a lint rule's ignores ([@ijlee2](https://github.com/ijlee2))

## 0.9.4

### Patch Changes

- [#2130](https://github.com/ember-intl/ember-intl/pull/2130) Set target to esnext ([@ijlee2](https://github.com/ijlee2))
- [#2129](https://github.com/ember-intl/ember-intl/pull/2129) Updated dependencies ([@ijlee2](https://github.com/ijlee2))

## 0.9.3

### Patch Changes

- [#2125](https://github.com/ember-intl/ember-intl/pull/2125) Updated dependencies ([@ijlee2](https://github.com/ijlee2))

## 0.9.2

### Patch Changes

- [#2123](https://github.com/ember-intl/ember-intl/pull/2123) Updated dependencies ([@ijlee2](https://github.com/ijlee2))

## 0.9.1

### Patch Changes

- [#2121](https://github.com/ember-intl/ember-intl/pull/2121) Standardized usage of @codemod-utils/threads ([@ijlee2](https://github.com/ijlee2))

## 0.9.0

### Minor Changes

- [#2119](https://github.com/ember-intl/ember-intl/pull/2119) Installed @codemod-utils/threads ([@ijlee2](https://github.com/ijlee2))

### Patch Changes

- [#2118](https://github.com/ember-intl/ember-intl/pull/2118) Refactored worker utilities ([@ijlee2](https://github.com/ijlee2))
- [#2117](https://github.com/ember-intl/ember-intl/pull/2117) Refactored worker utilities ([@ijlee2](https://github.com/ijlee2))

## 0.8.0

### Minor Changes

- [#2087](https://github.com/ember-intl/ember-intl/pull/2087) Improved error messages (no-inconsistent-messages) ([@johanrd](https://github.com/johanrd))

### Patch Changes

- [#2113](https://github.com/ember-intl/ember-intl/pull/2113) Updated dependencies ([@ijlee2](https://github.com/ijlee2))
- [#2110](https://github.com/ember-intl/ember-intl/pull/2110) Handled spaces in a folder name ([@ijlee2](https://github.com/ijlee2))

## 0.7.2

### Patch Changes

- [#2101](https://github.com/ember-intl/ember-intl/pull/2101) Updated dependencies ([@ijlee2](https://github.com/ijlee2))
- [#2100](https://github.com/ember-intl/ember-intl/pull/2100) Removed directories field (unused) ([@ijlee2](https://github.com/ijlee2))

## 0.7.1

### Patch Changes

- [#2092](https://github.com/ember-intl/ember-intl/pull/2092) Updated dependencies ([@ijlee2](https://github.com/ijlee2))

## 0.7.0

### Minor Changes

- [#2091](https://github.com/ember-intl/ember-intl/pull/2091) Parallelized findUsedKeys() ([@ijlee2](https://github.com/ijlee2))

## 0.6.1

### Patch Changes

- [#2090](https://github.com/ember-intl/ember-intl/pull/2090) Refactored LintRunWithIgnores ([@ijlee2](https://github.com/ijlee2))

## 0.6.0

### Minor Changes

- [#2089](https://github.com/ember-intl/ember-intl/pull/2089) Reported unused ignores ([@ijlee2](https://github.com/ijlee2))

### Patch Changes

- [#2088](https://github.com/ember-intl/ember-intl/pull/2088) Improved separation of responsibilities (create-options) ([@ijlee2](https://github.com/ijlee2))
- [#2088](https://github.com/ember-intl/ember-intl/pull/2088) Optimized calculation (no-inconsistent-messages) ([@ijlee2](https://github.com/ijlee2))
- [#2088](https://github.com/ember-intl/ember-intl/pull/2088) Updated test setups ([@ijlee2](https://github.com/ijlee2))
- [#2086](https://github.com/ember-intl/ember-intl/pull/2086) Specified return types ([@ijlee2](https://github.com/ijlee2))
- [#2086](https://github.com/ember-intl/ember-intl/pull/2086) Updated dependencies ([@ijlee2](https://github.com/ijlee2))
- [#2080](https://github.com/ember-intl/ember-intl/pull/2080) Added tests for pluralization ([@ijlee2](https://github.com/ijlee2))

## 0.5.0

### Minor Changes

- [#2074](https://github.com/ember-intl/ember-intl/pull/2074) Simplified analyze-project step ([@ijlee2](https://github.com/ijlee2))
- [#2071](https://github.com/ember-intl/ember-intl/pull/2071) Removed publicOnly ([@ijlee2](https://github.com/ijlee2))

### Patch Changes

- [#2075](https://github.com/ember-intl/ember-intl/pull/2075) Allowed each lint method to define a fix ([@ijlee2](https://github.com/ijlee2))
- [#2073](https://github.com/ember-intl/ember-intl/pull/2073) Simplified interfaces and removed unused code ([@ijlee2](https://github.com/ijlee2))
- [#2072](https://github.com/ember-intl/ember-intl/pull/2072) Simplified analyze-project step ([@ijlee2](https://github.com/ijlee2))

## 0.4.1

### Patch Changes

- [#2070](https://github.com/ember-intl/ember-intl/pull/2070) Improved log readability ([@ijlee2](https://github.com/ijlee2))

## 0.4.0

### Minor Changes

- [#2066](https://github.com/ember-intl/ember-intl/pull/2066) Supported fallbackLocale ([@ijlee2](https://github.com/ijlee2))
- [#2065](https://github.com/ember-intl/ember-intl/pull/2065) Allowed no-inconsistent-messages to ignore a translation's source ([@ijlee2](https://github.com/ijlee2))

### Patch Changes

- [#2064](https://github.com/ember-intl/ember-intl/pull/2064) Refactored noInconsistentMessages() ([@ijlee2](https://github.com/ijlee2))

## 0.3.3

### Patch Changes

- [#2063](https://github.com/ember-intl/ember-intl/pull/2063) Updated documentation ([@ijlee2](https://github.com/ijlee2))
- [#2062](https://github.com/ember-intl/ember-intl/pull/2062) Reverted normalizing locales to encourage explicit code ([@ijlee2](https://github.com/ijlee2))
- [#2060](https://github.com/ember-intl/ember-intl/pull/2060) Normalized locale (translation file name) ([@ijlee2](https://github.com/ijlee2))

## 0.3.2

### Patch Changes

- [#2058](https://github.com/ember-intl/ember-intl/pull/2058) Moved files ([@ijlee2](https://github.com/ijlee2))

## 0.3.1

### Patch Changes

- [#2054](https://github.com/ember-intl/ember-intl/pull/2054) Added tests for buildOptions ([@ijlee2](https://github.com/ijlee2))
- [#2053](https://github.com/ember-intl/ember-intl/pull/2053) Allowed passing fallbackLocale and publicOnly to buildOptions ([@ijlee2](https://github.com/ijlee2))

## 0.3.0

### Minor Changes

- [#2048](https://github.com/ember-intl/ember-intl/pull/2048) Simplified LintMethod ([@ijlee2](https://github.com/ijlee2))

### Patch Changes

- [#2050](https://github.com/ember-intl/ember-intl/pull/2050) Updated dependencies ([@ijlee2](https://github.com/ijlee2))
- [#2049](https://github.com/ember-intl/ember-intl/pull/2049) Updated copyright year ([@ijlee2](https://github.com/ijlee2))
- [#2048](https://github.com/ember-intl/ember-intl/pull/2048) Added integration tests ([@ijlee2](https://github.com/ijlee2))
- [#2047](https://github.com/ember-intl/ember-intl/pull/2047) Added acceptance tests ([@ijlee2](https://github.com/ijlee2))

## 0.2.2

### Patch Changes

- [#2039](https://github.com/ember-intl/ember-intl/pull/2039) Refactored code ([@ijlee2](https://github.com/ijlee2))
- [#2038](https://github.com/ember-intl/ember-intl/pull/2038) Refactored code ([@ijlee2](https://github.com/ijlee2))
- [#2036](https://github.com/ember-intl/ember-intl/pull/2036) Specified repository.directory for npm ([@ijlee2](https://github.com/ijlee2))
- [#2036](https://github.com/ember-intl/ember-intl/pull/2036) Updated README ([@ijlee2](https://github.com/ijlee2))

## 0.2.1

### Patch Changes

- [#2035](https://github.com/ember-intl/ember-intl/pull/2035) Handled top-level translation files with wrapTranslationsWithNamespace: true ([@ijlee2](https://github.com/ijlee2))

## 0.2.0

### Minor Changes

- [#2031](https://github.com/ember-intl/ember-intl/pull/2031) Supported Windows ([@ijlee2](https://github.com/ijlee2))

### Patch Changes

- [#2032](https://github.com/ember-intl/ember-intl/pull/2032) Updated dependencies ([@ijlee2](https://github.com/ijlee2))
- [#2030](https://github.com/ember-intl/ember-intl/pull/2030) Fixed typos ([@ijlee2](https://github.com/ijlee2))

## 0.1.0

### Minor Changes

- [#2026](https://github.com/ember-intl/ember-intl/pull/2026) Supported addons and build options ([@ijlee2](https://github.com/ijlee2))
- [#2026](https://github.com/ember-intl/ember-intl/pull/2026) Updated the configuration type and its expected location ([@ijlee2](https://github.com/ijlee2))
- [#2024](https://github.com/ember-intl/ember-intl/pull/2024) Created lint rule (no-inconsistent-messages) ([@ijlee2](https://github.com/ijlee2))
- [#2023](https://github.com/ember-intl/ember-intl/pull/2023) Stored additional information in Project ([@ijlee2](https://github.com/ijlee2))
- [#2023](https://github.com/ember-intl/ember-intl/pull/2023) Renamed lint rules ([@ijlee2](https://github.com/ijlee2))
- [#2020](https://github.com/ember-intl/ember-intl/pull/2020) Scaffolded package ([@ijlee2](https://github.com/ijlee2))

### Patch Changes

- [#2027](https://github.com/ember-intl/ember-intl/pull/2027) Updated README ([@ijlee2](https://github.com/ijlee2))
- [#2026](https://github.com/ember-intl/ember-intl/pull/2026) Updated noInconsistentMessages() to do a locale-based match ([@ijlee2](https://github.com/ijlee2))
- [#2022](https://github.com/ember-intl/ember-intl/pull/2022) Added tests for utilities ([@ijlee2](https://github.com/ijlee2))
