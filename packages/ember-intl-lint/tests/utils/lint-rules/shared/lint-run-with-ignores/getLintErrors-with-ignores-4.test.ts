...
test('utils | lint-rules | shared | lint-run-with-ignores > getLintErrors with ignores (4)', function () {
  const lintRun = new LintRunWithIgnores({
    ignores: ['key01', /^global\./g],
  ...
  assert.deepStrictEqual(lintRun.getLintErrors(), ['key02', 'global.03']);
